...
class IntermediaryCodeTestSuite extends AnyFlatSpec {
    ...
    ("basic/exit/exit-1.wacc") should "have intermediary code correctly produced by our translator" in {
        ...
        val intermediaryCode = getintermediaryCode(testFile)
        val expectedCode = Program(List(InbuiltFunction(A_Exit, Stored("1"))), List())
        assert(intermediaryCode == expectedCode)
    }
    ...
    //     // int x = 5
    //     expectedMain = expectedMain ++ List(UnaryOperation(A_Assign, Stored("1"), Stored("x0")))
    //     // int y = 3
    //     expectedMain = expectedMain ++ List(UnaryOperation(A_Assign, Stored("3"), Stored("y0")))
    //     // println x / y
    //     // expectedMain = expectedMain ++ List(InbuiltFunction(A_Println))
...
//         val intermediaryCode = getintermediaryCode(testFile)
//         println("intermediary code: ", intermediaryCode)
//         val expectedCode = Program(List(), List()) // [A_Assign, A_Neg, A_Exit])
